								...
								<FloatingCard delay={0.5}>
									<div className="relative">
										<div className="transform rotate-[-5deg] hover:rotate-0 transition-transform duration-500 z-10 relative">
											<PostCard
												author={{ name: "Campus Arts", handle: "arts_club", avatar: "", role: "Club" }}
...
function WorkflowStep({ number, title, desc, delay }: { number: string, title: string, desc: string, delay: number }) {
	return (
		...
		>
			<div className="h-16 w-16 rounded-full bg-card border-4 border-background shadow-xl flex items-center justify-center text-2xl font-bold text-primary mb-6 relative">
				{number}
...
function StatItem({ value, suffix = "", label }: { value: number, suffix?: string, label: string }) {
	...
	return (
		<motion.div
			...
			transition={{ duration: 0.5 }}
		>
			<div ref={ref} className="text-4xl md:text-5xl font-extrabold mb-2 text-white">0{suffix}</div>
